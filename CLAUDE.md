...
**베팅 상세 금액 표기**: 당첨 판정은 베팅 방식별(`bet_type`: exact=정확 스코어 일치, wdl=승/무/패 방향 일치).
- 진행중(open/closed) 리스트: 스코어별 "**예상** 인당 n원"(그 스코어가 맞을 경우 가정)
- 종료(ended) 리스트: **당첨 스코어에만** "🏆 당첨 인당 n원"(입금확인 판돈 ÷ 입금확인 당첨자, 실제 금액) — 낙첨 스코어엔 금액 미표기
