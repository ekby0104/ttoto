...
**베팅 가능(open) 카드/리스트 행**: 흰 배경 + 초록 반짝 보더(`greenBlink`) — `.bk-card.bettable, .ovvbet, .grp-game.gg-bet`
...
**베팅 상세 화살표 플로트(`#g-hint`)**: open 경기에서 베팅하기 버튼(`#g-betbtn`)이 스크롤 밖일 때만 ⌄⌄⌄ **세로 쉐브론 캐스케이드**(확정 디자인, 가로 ↓↓↓ 아님) 표시. 탭=버튼으로 스크롤(smooth+즉시 폴백), 버튼 보이면 자동 숨김
